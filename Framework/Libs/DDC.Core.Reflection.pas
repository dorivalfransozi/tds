...
{$IFEND}
  ...
  public
    function GetAttributeOfType<T: TCustomAttribute>: T; deprecated;
    function GetAttributesOfType<T: TCustomAttribute>: TArray<T>; deprecated;
    ...
    function HasAttributeOfType<T: TCustomAttribute>: Boolean; deprecated;
    function IsDefined<T: TCustomAttribute>: Boolean; overload;
    function TryGetAttributeOfType<T: TCustomAttribute>(out AAttribute: T): Boolean; deprecated;
  ...
  public
    function GetAllAttributes(attributeType: TClass; Inherit: Boolean = False): TArray<TCustomAttribute>; overload;
    function GetAllAttributes<T: TCustomAttribute>(Inherit: Boolean = False): TArray<T>; overload;

    function GetCustomAttribute<T: TCustomAttribute>(Inherit: Boolean = False): T;
    function GetCustomAttributes(Inherit: Boolean = False): TArray<TCustomAttribute>; overload;
    function GetCustomAttributes(attributeType: TClass; Inherit: Boolean = False): TArray<TCustomAttribute>; overload;
    function GetCustomAttributes<T: TCustomAttribute>(Inherit: Boolean = False): TArray<T>; overload;
  ...
  public
    ...
    class function FindByName(Parent: TRttiType; const PropertyName: string): TRttiPropertyExtension; overload;
...
{$IFEND}
  ...
  TEnum = record
  public
    class function AsInteger<T>(Value: T): Integer; static;
    class function AsString<T>(Value: T): string; static;
  end;
...
begin
  ...
  begin
    ...
    Move(LValue.GetReferenceToRawData^, LBuffer[0],
      LType.TypeSize - SizeOf(string));
...
var
  ...
  i: Integer;
begin
  Result := TryGetRttiType(ATarget, LType) and
    LType.AsInstance.MetaclassType.InheritsFrom(TStrings);
  if Result then
  begin
...
begin
  ...
  begin
    ...
    else if TryGetRttiType(ASource.TypeInfo, LSourceType) and
      LSourceType.IsGenericTypeOf('IList') then
    begin
      if (ATarget.Name = 'IList') and LSourceType.TryGetMethod('AsList', LMethod)
      then
      begin
        ...
        if TryGetRttiType(ATarget, LTargetType) and
        LTargetType.IsGenericTypeOf('IList') then
      begin
...
var
  ...
  LValue: TValue;
begin
  Result := TryGetRttiType(ASource.TypeInfo, LType) and
    LType.IsGenericTypeOf('Nullable');
  if Result then
  begin
    LValue := TValue.From(ASource.GetReferenceToRawData,
      LType.GetGenericArguments[0].Handle);
...
begin
  AResult := TValue.FromOrdinal(ATarget, GetEnumValue(ATarget,
    ASource.AsString));
  Result := True;
end;
...
begin
  ...
  for LType in Context.GetTypes do
  begin
    if (LType is TRttiInterfaceType) and
      IsEqualGUID(TRttiInterfaceType(LType).GUID, AGuid) then
    begin
      AType := LType;
      Break;
    end;
...
function TRttiMemberHelper.GetAllAttributes(attributeType: TClass; Inherit: Boolean): TArray<TCustomAttribute>;
begin
...
function TRttiMemberHelper.GetCustomAttributes(Inherit: Boolean): TArray<TCustomAttribute>;
begin
  Result := GetCustomAttributes(TCustomAttribute, Inherit);
end;

function TRttiMemberHelper.GetCustomAttributes(attributeType: TClass; Inherit: Boolean): TArray<TCustomAttribute>;
var
  LMethod: TRttiMethod;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (LMethod <> Self) and (LMethod.Name = Self.Name) and
          (LMethod.ReturnType = TRttiMethod(Self).ReturnType) and
          TRttiParameter.Equals(LMethod.GetParameters,
          TRttiMethod(Self).GetParameters) then
        begin
          Result := TArrayHelper.Concat<TCustomAttribute>
            ([Result, LMethod.GetCustomAttributes(attributeType, Inherit)]);
        end;
      end;
    end
  end;
...
begin
  ...
  begin
    ...
    begin
      if (Left[i].ParamType <> Right[i].ParamType) or
        (Left[i].Flags <> Right[i].Flags) then
      begin
        Result := False;
        Break;
      end;
...
function TRttiTypeHelper.GetCustomAttributes(attributeType: TClass; Inherit: Boolean): TArray<TCustomAttribute>;
var
  LAttribute: TCustomAttribute;
begin
  SetLength(Result, 0);
...
begin
  ...
  begin
    if SameText(GetGenericTypeDefinition, T.GetGenericTypeDefinition) or
      SameText(GetGenericTypeDefinition(False),
      T.GetGenericTypeDefinition(False)) then
    begin
      Result := True;
      ...
      for i := Low(Args) to High(Args) do
      begin
        if Args[i].IsInterface and Args[i].IsInterface and
          Args[i].InheritsFrom(otherArgs[i].Handle) then
        begin
          Continue;
        end;

        if Args[i].IsInstance and otherArgs[i].IsInstance and
          Args[i].InheritsFrom(otherArgs[i].Handle) then
        begin
          Continue;
        end;
...
begin
  S := Name;
  Result := (Copy(S, 1, Succ(Length(BaseTypeName))) = (BaseTypeName + '<')) and
    (Copy(S, Length(S), 1) = '>');
end;
...
var
  LType: TRttiType;
begin
  Result := SameText(Name, OtherTypeName) or
    (IsPublicType and SameText(QualifiedName, OtherTypeName));
...
begin
  ...
  begin
    ...
    S := Copy(QualifiedName, 1, Pos('<', QualifiedName)) +
      MergeStrings(Args, ',') + '>';
...
begin
  Result := Kind in [tkChar, tkString, tkWChar, tkLString, tkWString,
    tkUString];
end;
...
class function TValueHelper.ToString(const Values: array of TValue; Index: Integer): string;
var
  i: Integer;
begin
  Result := '';
...
begin
  ...
  FRegister := TObjectDictionary<TPair<PTypeInfo, string>,
    TRttiPropertyExtension>.Create([doOwnsValues]);
end;
...
begin
  ...
  begin
    if SameText(LProp.Name, LName) and LProp.Parent.IsInstance and
      EndsText(LScope, LProp.Parent.AsInstance.MetaclassType.QualifiedClassName)
    then
    begin
      Result := LProp;
      Break;
...
class function TEnum.AsInteger<T>(Value: T): Integer;
begin
  case SizeOf(T) of
...
class function TEnum.AsString<T>(Value: T): string;
begin
  Result := GetEnumName(TypeInfo(T), AsInteger(Value));
end;
